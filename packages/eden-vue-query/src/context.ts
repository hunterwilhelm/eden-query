...
export type EdenQueryUtils<TRouter extends AnyElysia> = {
  ...
  /**
   * @link https://tanstack.com/query/v5/docs/framework/vue/guides/prefetching
   */
  prefetchQuery: (
    queryKey: EdenQueryKey,
    opts?: EdenFetchQueryOptions<unknown, EdenClientError<TRouter>>,
  ...
  /**
   * @link https://tanstack.com/query/v5/docs/framework/vue/guides/query-invalidation
   */
  invalidateQueries: (
    queryKey: EdenQueryKey,
  ...
  /**
   * @link https://tanstack.com/query/v5/docs/framework/vue/guides/query-cancellation
   */
...
export const EdenQueryInjectionKey = Symbol('EdenQueryInjectionKey')

export type SSRState = 'mounted' | 'mounting' | 'prepass' | false

export type EdenContextPropsBase<TElysia extends AnyElysia = AnyElysia> = {
  /**
   * The `TRPCClient`
   */
  ...
  /**
   * @deprecated pass abortOnUnmount to `createTRPCVue` instead
...
export type EdenProviderProps<TRouter extends AnyElysia> = EdenContextProps<
  TRouter
>

export type EdenProvider<TRouter extends AnyElysia> = (
  props: EdenProviderProps<TRouter>,
) => Plugin

export type EdenContextProps<TRouter extends AnyElysia> = EdenContextPropsBase<
  TRouter
> & {
  /**
   * The vue-query `QueryClient`
   */
  queryClient: QueryClient
}

export type EdenContextState<TRouter extends AnyElysia> = Required<
  EdenContextProps<TRouter>
> &
  EdenQueryUtils<TRouter>

export const contextProps: (keyof EdenContextPropsBase<any>)[] = [
  'client',
  'abortOnUnmount',
]
...
/**
 * Creates a set of utility functions that can be used to interact with `vue-query`
 * @param options the `TRPCClient` and `QueryClient` to use
 * @returns a set of utility functions that can be used to interact with `vue-query`
 * @internal
 */
export function createUtilityFunctions<T extends AnyElysia>(
  options: EdenContextProps<T>,
