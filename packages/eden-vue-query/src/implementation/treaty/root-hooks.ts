...
import {
  createTreatyVueQueryCreateQueriesProxy,
  type EdenTreatyVueQueryCreateQueries,
} from './create-queries'
import { createEdenTreatyQueryUtils, type EdenTreatyVueQueryUtils } from './query-utils'
...
>(config?: EdenQueryConfig<TElysia>) {
  ...
  const setContext = (
    ...
    const context = createContext(props, configOverride)
    provide(EdenQueryContext, reactive(context))
    return context
  }
  ...
  const getRawContext = (): ProviderContext => {
    ...
    if (!instance) {
      throw new Error('getRawContext must be called within a setup function')
    }
    const context = inject<ProviderContext>(EdenQueryContext)
    if (!context) {
      throw new Error('EdenQueryContext not found')
    }
    return context
  }
  ...
  const createQuery = (
    ...
    const inputStore = isStore(input) ? input : reactive(input)

    const optionsStore = isStore(options) ? options : reactive(options)

    const queryOptionsStore = computed(() => {
      const queryOptions = edenCreateQueryOptions(parsed, context, inputStore, optionsStore, config)
      return queryOptions
    })
  ...
  const createQueries: EdenTreatyVueQueryCreateQueries<TElysia> = (queriesCallback) => {
    const context = getRawContext()

    const { queryClient, client } = context

    const proxy = createTreatyVueQueryCreateQueriesProxy(client)
  ...
  const createInfiniteQuery = (
    ...
    const queryOptionsStore = computed(() => {
      const queryOptions = edenCreateInfiniteQueryOptions(parsed, context, inputStore, optionsStore, config)
      return queryOptions
    })
  ...
  const createMutation = (
    ...
    const mutationOptionsStore = computed(() => {
      const mutationOptions = edenCreateMutationOptions(parsed, context, optionsStore, config)
