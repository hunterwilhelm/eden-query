...
import {
  ...
  type UseMutationReturnType as UseMutationResult, // type UseMutationResult,
  useQueryClient,
...
export function getEdenUseMutationOptions(
  ...
  const mutationOptions: UseMutationOptions = {
    ...
    onSuccess: (data, variables, context) => {
      const optionsValue = isRef(options) ? options.value : options
      const onSuccess = optionsValue?.onSuccess ?? defaultOptions.onSuccess
      const onSuccessValue = isRef(onSuccess) ? onSuccess.value : onSuccess

      if (config?.overrides?.useMutation?.onSuccess == null) {
        return onSuccessValue?.(data, variables, context)
      }

      const meta = optionsValue?.meta ?? defaultOptions.meta
      const metaValue = isRef(meta) ? meta.value : meta

      const originalFn = () => onSuccessValue?.(data, variables, context)

      return config.overrides.useMutation.onSuccess({ meta: metaValue, originalFn, queryClient })
    },
  }
